import type { VolunteerResponse, BeneficiaryCommitment, NeedStatusUpdate } from '@/types/response';
...
export function getResponsesForFunction(functionId: string): VolunteerResponse[] {
  return mockVolunteerResponses.filter(r => r.functionId === functionId);
}

export function getCommitmentsForVenue(venueId: string): BeneficiaryCommitment[] {
  return mockBeneficiaryCommitments.filter(c => c.venueId === venueId);
}

export function getCommitmentsForFunction(functionId: string): BeneficiaryCommitment[] {
  return mockBeneficiaryCommitments.filter(c => c.functionId === functionId);
}

export function getNeedStatus(functionId: string, categoryId?: string, serviceType?: string): NeedStatusUpdate | undefined {
  return mockNeedStatusUpdates.find(
    s => s.functionId === functionId && 
         (categoryId ? s.itemCategoryId === categoryId : s.serviceType === serviceType)
  );
}
